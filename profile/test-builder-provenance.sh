...
for image in $(jq -rc '.functions[].image' catalog.json); do
    digest=$(jq -r --arg image "$image" '.functions[] | select(.image | contains($image)) | .digest' catalog.json)
    builder=$(jq -r --arg image "$image" '.functions[] | select(.image | contains($image)) | .builder' catalog.json)
